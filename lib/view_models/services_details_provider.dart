...
import 'package:salon_customer_app/models/dashboard_models/sub_service_detail_model.dart';
...
class ServicesDetailsProvider extends ChangeNotifier {
  ...
  SubServiceDetailModel _subServiceDetail=SubServiceDetailModel();
  SubServiceDetailModel get subServiceDetail=> _subServiceDetail;
  ...
  Future<bool> subServiceDetails({
    ...
    try {
      var state = AuthProvider(await SharedPreferences.getInstance());
      var res = await ApiClient.postApi(
        url: appUrls.getSubServiceDetailUrl,
        body: body,
        headers: {
          'Authorization': 'Bearer ${state.userData.token ?? ''}',
        },
      );
      _subServiceDetail = SubServiceDetailModel.fromJson(res?.data);
      _setShowLoader(false);
      notifyListeners();
      return true;
    } catch (e) {
